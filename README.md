...
2. Add `minimal` to the module setup configuration in your `.zimrc`:
    ...
    zstyle ':zim:module' minimal 'url' 'minimal'

3. Execute `zimfw install` to install the prompt.
...
* Full `promptinit` compatibility, but can also be sourced standalone.
...
Adding functionality is as easy as writing a shell function and adding it to one of the arrays in your `.zshrc`:
